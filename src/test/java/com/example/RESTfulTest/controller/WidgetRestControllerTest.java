...
class WidgetRestControllerTest {
    ...
    void testUpdateWidget() throws Exception {
        // Setup our mocked service
        Widget widgetToPost = new Widget(1L, "New Widget", "This is my widget", 1);
        Widget widgetToReturn = new Widget(1L, "Widget Anew", "This is my widget", 1);
        when(service.save(any())).thenReturn(widgetToPost);
        when(service.findById(any())).thenReturn(Optional.of(widgetToReturn));
    ...
    void testUpdateWidgetByIdNotFound() throws Exception {
        // Setup our mocked service
        Widget widgetToPost = new Widget(2L, "New Widget", "This is my widget", 1);
        Optional<Widget> widgetToReturn = Optional.empty();
        when(service.save(any())).thenReturn(widgetToPost);
        when(service.findById(any())).thenReturn(widgetToReturn);

        // Execute the PUT request
        mockMvc.perform(put("/rest/widget/{id}", 2)
                        .header(HttpHeaders.IF_MATCH, "1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(asJsonString(widgetToPost)))
                        .andExpect(status().isNotFound());
    ...
    void testGetWidget() throws Exception {
        ...
        mockMvc.perform(get("/rest/widget/{id}", 1))
                ...
                // Validate the returned fields
                .andExpect(jsonPath("$.id", is(1)))
                .andExpect(jsonPath("$.name", is("Widget Anew")))
                .andExpect(jsonPath("$.description", is("This is my widget")))
                .andExpect(jsonPath("$.version", is(1)));
    }
